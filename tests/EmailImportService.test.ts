...
import {
  generateOneThreadEmails,
  generateTwoThreadEmails,
} from "./helpers/TestEmails";
...
describe("EmailImportService", () => {
  ...
  it("should process emails into one thread", async () => {
    ...
    expect(mockDependencies.emailRepository.persist).toHaveBeenCalledWith(
      fetchedEmails,
    );
    expect(mockDependencies.threadRepository.persist).toHaveBeenCalledTimes(1); // One thread should be created

    const createdMessages = mockDependencies.messageRepository.persist.mock
      .calls[0][0] as MessageEntity[];
